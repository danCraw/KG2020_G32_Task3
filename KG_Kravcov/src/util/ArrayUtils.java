...
public class ArrayUtils {
    ...
    public static Map<String, Double> arrToMap(Object[][] arr) {
        Map<String, Double> map = new HashMap<>();
        for (int i = 0; i < arr.length; i++) {
            for (int j = 0; j < 1; j++) {
                map.put((String) arr[i][0], Double.parseDouble((String) arr[i][1]));
            }
        }
        return map;
    }

    private static Object[][] arrayListToObjectArr(ArrayList arrayList) {
        int size = 0;
        ...
        for (int i = 0; i < size; i++) {
            arr[i][0] = arrayList.get(i);
            arr[i][1] = arrayList.get(i + 1);
        }
        return arr;
    }

    public static Object[][] removeElementFromArray(Object[][] arr, Object key) {
        ArrayList<Object> arrayList = new ArrayList<>();
        for (Object[] objects : arr) {
            if (objects[0] != key) {
                arrayList.add(objects[0]);
                arrayList.add(objects[1]);
            }
        }
        return arrayListToObjectArr(arrayList);
    }

    public static Object[][] mapToObjectArray(Map<String, Double> map) {
        Object[][] arr = new Object[map.size()][2];
        for (int i = 0; i < arr.length; i++) {
            for (int j = 0; j < 2; j++) {
                arr[i][0] = map.keySet().toArray()[i];
                arr[i][1] = map.get(map.keySet().toArray()[i]);
            }
        }
        return arr;
    }

    public static Object[] twoDimensionalToOneArray(Object[][] twoArr) {

        Object[] smallArray = new Object[twoArr[0].length + twoArr[1].length];

        for (int i = 0; i < smallArray.length; i++) {
            for (int j = 0; j < smallArray.length; j++) {
                smallArray[i] = String.valueOf(twoArr[i][j]);
            }
        }
        return smallArray;
    }
